...
pub fn get_log_dir() -> PathBuf {
    let log_dir = if cfg!(windows) {
        // Windows: %TEMP%/NoxumLauncher/logs
        if let Ok(temp) = std::env::var("TEMP") {
            PathBuf::from(temp).join("NoxumLauncher").join("logs")
        } else {
            get_base_path().join("logs")
        }
    } else {
        // Linux/macOS: ~/.local/share/noxum-launcher/logs или ./logs
        if let Ok(home) = std::env::var("HOME") {
            let dir = PathBuf::from(home).join(".local").join("share").join("noxum-launcher").join("logs");
            if dir.exists() {
                dir
            } else {
                get_base_path().join("logs")
